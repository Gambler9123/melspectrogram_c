...
// 来自知乎
std::vector<std::vector<float>> Lib_Mfcc::STFT(std::vector<float> audio, int hopLength, int n_fft)
{
    using namespace std::complex_literals;
    ...
    for (int i = 0; i < y_frams; i++) {

        std::vector<float> audioData(n_fft);

        for (int n = 0; n < (n_fft); n++) {
            float hannWindowMultiplier = (float)(0.5 * (1.0 - cos(2.0 * pi * n / ((float)n_fft))));               // hanning window
            audioData[n] = hannWindowMultiplier * audio[n + (hopLength * i)];
        }
...
std::vector<std::complex<float>> Lib_Mfcc::getFFT(std::vector<float> audio)
{
    using namespace std::complex_literals;
    auto N = audio.size();
    std::vector<std::complex<float>> phasor(N / 2);
...
std::vector<std::vector<float>> Lib_Mfcc::getMelspectrogram(std::vector<std::vector<float>> fft, std::vector<std::vector<float>> mel_basis)
{
    std::vector<std::vector<float>> signal_power(fft[0].size(), std::vector<float>(fft.size()));    
    ...
    auto mel_spectrogram = dotProduct(mel_basis, signal_power);

    return mel_spectrogram;
}


std::vector<std::vector<float>> Lib_Mfcc::domelspectrogram(std::vector<float> audio, int hopLength, int n_fft,double sampleRate)
{
 
    auto stft_matrix = Lib_Mfcc::STFT(audio, hopLength, n_fft);
    
    auto weights = getMelFilterBank(sampleRate, n_fft);

    auto mel_spectrogram = getMelspectrogram(stft_matrix, weights);
